...
\begin{enumerate}[parsep=0.5ex]
    \item \textbf{Agent 工作流与智能选题}：基于 LangGraph 编排事件治理与文案生成等复杂 Agent 子流程，结合 PostgreSQL 持久化任务状态、调度器、Worker 与租约机制，串联新闻采集、智能选题、文案配图及内部交付；设计“规则阈值过滤 + LLM 受控重排”机制，通过优先级屏障、结果校验和确定性回退约束模型决策。
