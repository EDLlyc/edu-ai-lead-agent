...
- [x] Mark migration review complete with `previous_application_compatible=false`; prove the exact
...
- [x] Run focused unit/contract tests for settings, artifacts, worker, adapter, Compose, release,
      and doctor.
- [x] Run the previously green PostgreSQL/migration/full candidate gates plus final focused strict
      Ruff/mypy, 66 adapter/settings/CLI regressions, 54 release tests, Compose/image probes, shell
      syntax, secret scan, task validation, and `git diff --check`. Re-attempt the current full gate
      and record its unrelated pre-existing format/type/private-fixture failures in `result.md`.
- [x] Run `trellis-check`; fix verified findings and rerun affected gates.
- [x] Stage only this task's paths around the existing dirty workspace, commit coherent code/spec
...
- [x] Capture and checksum-bind the read-only production `0036` baseline; accept an absent weekly
      volume as zero during preflight and create required named volumes only after activation.
- [x] Derive clean Codeup source/image manifests and current Compose entrypoints including the
...
- [x] Exercise fake preflight/backup/activation/recovery cases, including migration failure,
...
- [x] Commit/push the reviewed task-local release evidence if it changes the authoritative operator;
      rebuild the candidate whenever the final commit changes.

## Phase 5: Production preflight and activation

- [x] Fetch exact Codeup SHA; run read-only server preflight and two stable samples. Verify current
...
- [x] Derive the first Monday strictly after activation in Asia/Shanghai and bind it into the
      operator/environment evidence.
- [x] Transfer protected artifacts, load only the isolated candidate, and invoke the physical
      mode-0600 operator once with null stdin through the server-owned transient boundary.
- [x] Quiesce, back up, activate the isolated source/image with no additional migration, verify the
      existing `0042` head and ordinary services, then atomically enable/start the optional worker.
- [x] Require an empty historical inbox, zero draft jobs/items/attempts/provider writes at
      activation, worker and ordinary service health/restart-zero, safe logs, exact head, and
      immediate/30-second stable evidence.

## Phase 6: Closeout

- [x] Record exact Codeup SHA, candidate/operator/source hashes, backup ID, migration result,
      cutoff, service matrix, safe job counters, provider-zero evidence, and recovery disposition.
- [x] If code/evidence changes after activation, commit/push evidence only and do not redeploy it.
