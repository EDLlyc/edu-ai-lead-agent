...
readonly RELEASE_HEAD_REF=refs/heads/release/substantive-news-scope-20260905
readonly RELEASE_REF=refs/remotes/origin/release/substantive-news-scope-20260905
...
readonly TASK_PATH=.trellis/tasks/09-05-production-recovery-qwen-embedding/research/substantive-release
readonly BUILDER_NAME=build-substantive-release.sh
readonly CAPTURE_NAME=capture-substantive-production-baseline.sh
...
readonly -a FINAL_MEMBERS=(
  ...
  capture-substantive-production-baseline.sh image-source.sha256
...
log() { printf '[substantive-release-builder] %s\n' "$*" >&2; }
...
cleanup() {
  ...
  if [[ -n "${scratch:-}" && "$scratch" == /tmp/edu-ai-substantive-release-build.?????? ]]; then
...
usage() {
  printf '%s\n' \
    "Usage: $BUILDER_NAME --release-sha HEX40 --main-policy-commit HEX40 --main-tooling-commit HEX40 --production-baseline ABSOLUTE_MODE_0600_JSON --scheduler-cutoff-utc YYYY-MM-DDTHH:MM:SSZ --output-dir ABSENT_ABSOLUTE_DIR" >&2
}
...
parse_args() {
  while (($#)); do
    case "$1" in
      ...
      --main-policy-commit) (($# >= 2)) || die 'missing main policy commit'; main_policy_commit=$2; shift 2 ;;
      --main-tooling-commit) (($# >= 2)) || die 'missing main tooling commit'; main_tooling_commit=$2; shift 2 ;;
  ...
  [[ "$release_sha" != "$PRODUCTION_COMMIT" ]] || die 'candidate must not be the current release'
  [[ "$main_policy_commit" =~ ^[0-9a-f]{40}$ ]] || die 'main policy commit must be lowercase HEX40'
  [[ "$main_tooling_commit" =~ ^[0-9a-f]{40}$ ]] \
    || die 'main tooling commit must be lowercase HEX40'
...
assert_authority() {
  ...
  git_clean -C "$repo_root" merge-base --is-ancestor "$main_policy_commit" "$main_sha" \
    || die 'main does not contain the reviewed fix commit'
  git_clean -C "$repo_root" merge-base --is-ancestor "$main_tooling_commit" "$main_sha" \
...
task_prefix = ".trellis/tasks/09-05-production-recovery-qwen-embedding/"
seen = {}
audit = {}
...
if not any(name.startswith(task_prefix) for name in audit) or any(
    audit.get(name) != status for name, status in audit_exact.items()
):
...
spec = importlib.util.spec_from_file_location("substantive_release_validator", validator_path)
...
write_metadata() {
  python3 - "$stage/release-metadata.json" "$release_sha" "$main_policy_commit" \
    "$main_tooling_commit" "$scheduler_cutoff_utc" "$candidate_repository" \
...
payload = {
    ...
    "main_policy_commit": main_fix,
    "main_tooling_commit": main_operator,
    ...
    "builder_sha256": digest("build-substantive-release.sh"),
    "capture_sha256": digest("capture-substantive-production-baseline.sh"),
    "operator_sha256": digest("substantive-release-operator.sh"),
    "validator_sha256": digest("validate-substantive-release.py"),
...
spec = importlib.util.spec_from_file_location("substantive_release_validator", sys.argv[1])
if spec is None or spec.loader is None:
    raise SystemExit("validator import failed")
module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(module)
...
main() {
  ...
  scratch=$(mktemp -d /tmp/edu-ai-substantive-release-build.XXXXXX)
  ...
  validate_staged_baseline
  python3 -B "$stage/$VALIDATOR_NAME" --window "$scheduler_cutoff_utc" \
    "$stage/production-baseline.json" >/dev/null
  capture_complete_diff
  validate_compose_entrypoints
  build_source_archive
  write_image_source_manifest
...
PY
  ...
  transport_tag="${candidate_repository}:substantive-release-${release_sha:0:12}"
  build_and_probe_image
  python3 -B "$stage/$VALIDATOR_NAME" --window "$scheduler_cutoff_utc" \
    "$stage/production-baseline.json" >/dev/null
  write_metadata
...
if [[ "${SUBSTANTIVE_RELEASE_BUILDER_SOURCE_ONLY:-0}" != 1 ]]; then
  main "$@"
fi
