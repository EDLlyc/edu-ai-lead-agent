...
if [[ "${SUBSTANTIVE_RELEASE_CAPTURE_SOURCE_ONLY:-0}" == 1 \
    && -n "${SUBSTANTIVE_RELEASE_CAPTURE_TEST_APP_DIR:-}" ]]; then
  capture_app_dir=$SUBSTANTIVE_RELEASE_CAPTURE_TEST_APP_DIR
fi
...
die() {
  printf '[substantive-release-baseline] ERROR: %s\n' "$*" >&2
  return 1
}
...
cleanup() {
  ...
  for path in "${source_json:-}" "${temporary:-}"; do
    ...
    case "$(basename -- "$path")" in
      .substantive-release-source.??????|.substantive-release-baseline.??????)
...
main() {
  while (($#)); do
    case "$1" in
      ...
      -h|--help)
        printf '%s\n' \
          'Usage: capture-substantive-production-baseline.sh --output ABSENT_ABSOLUTE_JSON' >&2
        return 0
        ;;
  ...
  [[ "$counts" =~ ^([0-9]+)(:[0-9]+){10}(:0){8}:112:196:196:95(:0){4}$ \
  ...
  [[ "$frozen_copy" == 7:657797a7d4b8d51c8355c07c62343610529fc85753ecb7b489dcd2ef3c5dc74c ]] \
  ...
  source_json=$(mktemp "$(dirname -- "$output")/.substantive-release-source.XXXXXX")
  rm -f -- "$source_json"
  ...
  temporary=$(mktemp "$(dirname -- "$output")/.substantive-release-baseline.XXXXXX")
...
PY
  chmod 600 "$temporary"
  python3 -B "$(dirname -- "${BASH_SOURCE[0]}")/validate-substantive-release.py" \
    --baseline "$temporary" >/dev/null
  mv -T "$temporary" "$output"
  rm -f -- "$source_json"
  printf 'substantive_production_baseline_captured output=%s\n' "$output"
}

if [[ "${SUBSTANTIVE_RELEASE_CAPTURE_SOURCE_ONLY:-0}" != 1 ]]; then
  trap cleanup EXIT
  main "$@"
fi
