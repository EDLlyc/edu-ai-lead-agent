#!/usr/bin/env bash
# Build the reviewed broad-release source/image artifacts from an already-fetched
...
readonly BASE_COMMIT="c66aa6217d137033118c552f3db11b2a1121d082"
...
readonly EXPECTED_BASE_SOURCE_FILE_COUNT=307
readonly EXPECTED_IMAGE_SOURCE_FILE_COUNT=179
readonly EXPECTED_BASE_IMAGE_SOURCE_FILE_COUNT=165
...
readonly TASK_RESEARCH_PATH=".trellis/tasks/08-17-deploy-delivered-repeat-window/research"
readonly BUILDER_REPO_PATH="${TASK_RESEARCH_PATH}/build-broad-offline-artifacts.sh"
readonly OPERATOR_REPO_PATH="${TASK_RESEARCH_PATH}/broad-offline-release-operator.sh"
readonly VALIDATOR_REPO_PATH="${TASK_RESEARCH_PATH}/validate-broad-offline-artifacts.py"
...
log() { printf '[broad-artifact-builder] %s\n' "$*" >&2; }
die() { log "ERROR: $*"; return 1; }

usage() {
  cat >&2 <<'EOF'
Usage: build-broad-offline-artifacts.sh --release-sha HEX40 [--output-dir ABSOLUTE_NEW_DIR]

The release SHA must already equal the locally fetched authoritative Codeup
refs/remotes/origin/main. The optional output path must not exist. On success,
...
parse_args() {
  ...
  [[ "$release_sha" =~ ^[0-9a-f]{40}$ ]] || { die "release SHA must be 40 lowercase hex characters"; return 2; }
...
assert_release_authority() {
  local root=$1 expected_sha=$2 origin_url origin_sha
  ...
  [[ "$origin_sha" =~ ^[0-9a-f]{40}$ && "$origin_sha" == "$expected_sha" ]] \
    || { die "release SHA is not the already-fetched authoritative origin/main"; return 1; }
  git_call -C "$root" cat-file -e "${BASE_COMMIT}^{commit}"
  git_call -C "$root" cat-file -e "${expected_sha}^{commit}"
}

assert_builder_authority() {
  local root=$1 expected_sha=$2 expected_path actual_path committed_hash actual_hash
  ...
  committed_hash=$(git_call -C "$root" show "${expected_sha}:${BUILDER_REPO_PATH}" | sha256sum | awk '{print $1}')
  actual_hash=$(sha256sum "$actual_path" | awk '{print $1}')
  [[ "$committed_hash" == "$actual_hash" ]] || { die "running builder differs from the authoritative release commit"; return 1; }
}
...
write_base_source_paths() {
  ...
  raw="${output}.raw"
  git_call -C "$root" ls-tree -r -z --name-only "$BASE_COMMIT" -- "${SOURCE_PATHSPECS[@]}" >"$raw"
  tr '\0' '\n' <"$raw" | LC_ALL=C sort >"$output"
  [[ "$(wc -l <"$output" | tr -d '[:space:]')" == "$EXPECTED_BASE_SOURCE_FILE_COUNT" ]] \
    || { die "c66 source baseline is not exact 307"; return 1; }
}
...
validate_reviewed_path_sets() {
  local base_paths=$1 candidate_paths=$2 additions_output=$3 expected_base=$4 expected_candidate=$5
  local removed additions path expected_additions
  [[ "$(wc -l <"$base_paths" | tr -d '[:space:]')" == "$expected_base" ]] \
    || { die "base path count mismatch"; return 1; }
  [[ "$(wc -l <"$candidate_paths" | tr -d '[:space:]')" == "$expected_candidate" ]] \
    || { die "candidate path count mismatch"; return 1; }
  [[ "$(LC_ALL=C sort -u "$base_paths")" == "$(<"$base_paths")" ]] \
    || { die "base paths are duplicate or unsorted"; return 1; }
  [[ "$(LC_ALL=C sort -u "$candidate_paths")" == "$(<"$candidate_paths")" ]] \
    || { die "candidate paths are duplicate or unsorted"; return 1; }
  removed=$(comm -23 "$base_paths" "$candidate_paths")
  [[ -z "$removed" ]] || { die "candidate removes a c66 source path"; return 1; }
  comm -13 "$base_paths" "$candidate_paths" >"$additions_output"
  additions=$(wc -l <"$additions_output" | tr -d '[:space:]')
  [[ "$additions" == "$((expected_candidate - expected_base))" ]] \
    || { die "candidate addition count mismatch"; return 1; }
  expected_additions=$(printf '%s\n' "${EXPECTED_SOURCE_ADDITIONS[@]}" | sed 's#^backend/##' | LC_ALL=C sort)
  if [[ "$expected_base" == "$EXPECTED_BASE_SOURCE_FILE_COUNT" ]]; then
    expected_additions=$(printf '%s\n' "${EXPECTED_SOURCE_ADDITIONS[@]}" | LC_ALL=C sort)
  fi
  [[ "$(<"$additions_output")" == "$expected_additions" ]] \
    || { die "candidate additions differ from the exact reviewed 14 paths"; return 1; }
  while IFS= read -r path; do
    if [[ "$expected_base" == "$EXPECTED_BASE_SOURCE_FILE_COUNT" ]]; then
      assert_safe_addition_path "$path"
    else
      assert_safe_addition_path "backend/$path"
    fi
...
assert_dependency_compatibility() {
  ...
  git_call -C "$repo_root" show "${BASE_COMMIT}:backend/pyproject.toml" >"$base_pyproject"
  ...
  cmp -- "$base_pyproject" <(git_call -C "$repo_root" show "${BASE_COMMIT}:backend/pyproject.toml") >/dev/null
  cmp -- "${worktree}/backend/requirements/runtime.lock" <(git_call -C "$repo_root" show "${BASE_COMMIT}:backend/requirements/runtime.lock") >/dev/null \
    || { die "production runtime lock changed from c66"; return 1; }
  cmp -- "${worktree}/backend/Dockerfile" <(git_call -C "$repo_root" show "${BASE_COMMIT}:backend/Dockerfile") >/dev/null \
    || { die "production Dockerfile changed from c66"; return 1; }
  python3 - "$base_pyproject" "${worktree}/backend/pyproject.toml" <<'PY'
import copy
import pathlib
import sys
import tomllib

base = tomllib.loads(pathlib.Path(sys.argv[1]).read_text(encoding="utf-8"))
final = tomllib.loads(pathlib.Path(sys.argv[2]).read_text(encoding="utf-8"))
normalized = copy.deepcopy(final)
dev = normalized["project"]["optional-dependencies"]["dev"]
if dev.count("mcp==2.0.0") != 1:
    raise SystemExit("final pyproject must add exactly mcp==2.0.0 to dev")
dev.remove("mcp==2.0.0")
pytest_config = normalized["tool"]["pytest"]["ini_options"]
if pytest_config.pop("pythonpath", None) != ["."]:
    raise SystemExit("final pyproject must add only pytest pythonpath=['.']")
if normalized != base:
    raise SystemExit("pyproject differs beyond dev-only mcp and pytest pythonpath")
if final["project"]["dependencies"] != base["project"]["dependencies"]:
    raise SystemExit("production dependencies changed")
PY
...
assert_candidate_labels() {
  ...
  for key in "${BROAD_LABEL_KEYS[@]}"; do
    case "$key" in
      ...
      *) die "unknown broad label key"; return 1 ;;
    esac
...
assert_candidate_runtime() {
  ...
  runtime_run --env 'CONTENT_SCORING_VERSION=scoring-v1-preview.6-tiered-science-tech-priority' \
    --entrypoint python "$candidate_id" -c \
    'from app.core.config import Settings; from app.application.services.topic_selection import build_topic_scoring_config; c=build_topic_scoring_config(Settings()); assert c.version=="scoring-v1-preview.6-tiered-science-tech-priority" and c.effective_veto_rule_version=="topic-veto-v3-governed-content"'
  runtime_run --env 'CONTENT_SCORING_VERSION=scoring-v1-preview.7-delivered-repeat-history' \
    --entrypoint python "$candidate_id" -c \
    'from app.core.config import Settings; from app.application.services.topic_selection import build_topic_scoring_config; c=build_topic_scoring_config(Settings()); assert c.version=="scoring-v1-preview.7-delivered-repeat-history" and c.effective_veto_rule_version=="topic-veto-v4-delivered-content"'
}
...
write_image_validation() {
  ...
  printf '%s\n' \
    ...
    'dev_dependency_delta=mcp==2.0.0' \
    'pytest_pythonpath=.' \
...
assert_stage_shape() {
  local stage=$1 actual expected member targets expected_targets
  ...
  for member in "${STAGE_MEMBERS[@]}"; do
    [[ -f "${stage}/${member}" && ! -L "${stage}/${member}" && "$(stat -c '%a' "${stage}/${member}")" == 600 ]] \
...
assemble_artifact_stage() {
  local generated=$1 output=$2 member
...
validate_completed_artifacts() {
  local stage=$1 validator
  validator="${stage}/validate-broad-offline-artifacts.py"
  ...
  python3 "$validator" image-source --observed "${stage}/image-source-files.sha256" \
...
cleanup() {
  ...
  if ((cleanup_armed)) && [[ -n "${scratch_root:-}" && "$scratch_root" == /tmp/edu-ai-broad-builder.* && -d "$scratch_root" && ! -L "$scratch_root" ]]; then
...
build_artifacts() {
  local base_paths candidate_paths source_additions base_image_paths candidate_image_paths
  local image_additions source_tree image_context source_paths_output source_modes_output
  local base_tree_index release_tree_index path relative git_mode base_mode candidate_manifest
  ...
  scratch_root=$(mktemp -d /tmp/edu-ai-broad-builder.XXXXXX)
  ...
  assert_release_authority "$repo_root" "$release_sha"
  assert_builder_authority "$repo_root" "$release_sha"
  ...
  load_tree_map "$repo_root" "$BASE_COMMIT" BASE "$base_tree_index"
  load_tree_map "$repo_root" "$release_sha" RELEASE "$release_tree_index"

  base_paths="${scratch_root}/base-source-paths"
  candidate_paths="${scratch_root}/candidate-source-paths"
  source_additions="${scratch_root}/source-additions"
  write_base_source_paths "$repo_root" "$base_paths"
  { cat "$base_paths"; printf '%s\n' "${EXPECTED_SOURCE_ADDITIONS[@]}"; } | LC_ALL=C sort -u >"$candidate_paths"
  validate_reviewed_path_sets "$base_paths" "$candidate_paths" "$source_additions" \
    "$EXPECTED_BASE_SOURCE_FILE_COUNT" "$EXPECTED_SOURCE_FILE_COUNT"

  while IFS= read -r path; do
    assert_safe_source_path "$path"
    [[ -n "${BASE_MODE[$path]+present}" && -n "${RELEASE_MODE[$path]+present}" ]] \
      || { die "candidate removes or replaces a c66 runtime blob"; return 1; }
    [[ "${BASE_MODE[$path]}" == "${RELEASE_MODE[$path]}" ]] \
      || { die "candidate changes a c66 runtime executable class"; return 1; }
  done <"$base_paths"
  for path in "${EXPECTED_SOURCE_ADDITIONS[@]}"; do
    assert_safe_addition_path "$path"
    [[ -z "${BASE_MODE[$path]+present}" && -n "${RELEASE_MODE[$path]+present}" ]] \
      || { die "reviewed source addition is absent or existed at c66"; return 1; }
  done

  base_image_paths="${scratch_root}/base-image-paths"
  candidate_image_paths="${scratch_root}/candidate-image-paths"
  image_additions="${scratch_root}/image-additions"
  write_image_scope_paths "$repo_root" "$BASE_COMMIT" "$base_image_paths"
  write_image_scope_paths "$repo_root" "$release_sha" "$candidate_image_paths"
  [[ "$(wc -l <"$base_image_paths" | tr -d '[:space:]')" == "$EXPECTED_BASE_IMAGE_SOURCE_FILE_COUNT" ]] \
    || { die "c66 image source baseline is not exact 165"; return 1; }
  validate_reviewed_path_sets "$base_image_paths" "$candidate_image_paths" "$image_additions" \
    "$EXPECTED_BASE_IMAGE_SOURCE_FILE_COUNT" "$EXPECTED_IMAGE_SOURCE_FILE_COUNT"
  ...
  python3 "${release_worktree}/${VALIDATOR_REPO_PATH}" source \
  ...
  dockerfile="${image_context}/Dockerfile.broad-offline"
  write_overlay_dockerfile "$dockerfile"

  candidate_tag="edu-ai-lead-agent-backend:broad-${release_sha:0:12}"
  build_candidate_image "$image_context" "$dockerfile"
  assert_candidate_runtime "$image_context" "${release_worktree}/${VALIDATOR_REPO_PATH}"
  ...
  python3 "${release_worktree}/${VALIDATOR_REPO_PATH}" image \
    --bundle "${artifact_stage}/backend-image.tar.gz" \
    --expected-tag "$candidate_tag" --expected-image-id "$candidate_id"

  install -m 0600 "${release_worktree}/${OPERATOR_REPO_PATH}" "${artifact_stage}/broad-offline-release-operator.sh"
  install -m 0600 "${release_worktree}/${VALIDATOR_REPO_PATH}" "${artifact_stage}/validate-broad-offline-artifacts.py"
  ...
  if [[ -n "$requested_output_dir" ]]; then
    ...
    final_output_dir=$(mktemp -d "/tmp/edu-ai-broad-artifacts-${release_sha:0:12}.XXXXXX")
    chmod 0700 "$final_output_dir"
  fi
  assemble_artifact_stage "$artifact_stage" "$final_output_dir"
...
if [[ "${BROAD_BUILDER_SOURCE_ONLY:-0}" != 1 ]]; then
  main "$@"
fi
