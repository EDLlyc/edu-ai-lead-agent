...
class DeterministicFakeMaterialDraftGenerator:
    ...
    async def generate(self, request: DraftGenerationRequest) -> DraftGenerationResult:
        ...
        body = (
            f"今天想和家长们分享一条值得关注的科技教育动态：{fact}🔎\n\n"
            f"{brand_statement}{opinion}"
            "学习人工智能和机器人，并不是先记住一串陌生名词，而是从身边的问题开始。孩子看到机器人"
            "怎样感知环境、判断方向、调整动作，就会自然地追问：它为什么这样做？🤖这些问题把抽象技术"
            "变成可以观察、讨论和动手验证的生活经验。科学学习的价值，不只在于得到答案，更在于提出"
            "问题、寻找证据，并在结果不如预期时重新尝试。这样的思考力，会帮助孩子理解身边的工具和"
            "社会变化，面对新技术时多一分好奇，少一分盲从。💡\n\n"
            "陪伴孩子学习不必从复杂术语开始。可以一起观察小装置，猜它怎样运动，再通过搭建、测试和"
            "修改验证想法。失败可以成为提问线索，创造是把想法变成方案并行动检验。🌱\n\n"
            "在赛先生学习，科学精神、好奇心、思考力和创造力会进入真实体验。课程鼓励孩子观察、动手实践、"
            "复盘改进，让人工智能和机器人成为理解世界、解决问题的工具。✨"
        )
        copywriting = f"{body}\n\n#赛先生科学 #人工智能启蒙 #科学思维"
        draft = MaterialDraft(
