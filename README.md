...
阈值和固定一次重生成上限。OCR 是独立可选门：`IMAGE_OCR_ENABLED=false` 时不会创建或调用
识别器，成图文字及顺序不做机器校验，但媒体、尺寸、供应商身份、按需启用的质量审计、相似度与
存储完整性门仍然执行。`make doctor` 会检查跨服务配置一致性。前端详情页仅供本地检查这些安全
标签，不进入生产部署。生产启用步骤见
...
对应短副标题。不得把原始新闻标题、完整文案、自由生成口号或伪文字画进图片；启用 OCR 时必须
只识别这三行，且标题卡不能遮挡角色面部、科学物体或主体动作。历史 v1 文字模式保持不变。
